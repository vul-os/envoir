# envoir-gateway — the legacy bridge (and its brief life as a separate repo)

**Status: FOLDED BACK IN (2026-07-17).** The gateway was split into a standalone
`envoir-gateway` repository (same day, earlier), pinning `dmtap-core`/`dmtap-mail` to monorepo
release tags. In practice `dmtap-core`'s API is still moving fast, and every core change that
