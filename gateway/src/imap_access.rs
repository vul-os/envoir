...
//! projection ([`dmtap_mail::store::MailStore`]) — the SAME projection the co-located node uses.
//!
...
use dmtap_mail::auth::Authenticator;
...
/// each session owns its own snapshot exactly as [`dmtap_mail::net::serve_imap`] does.
...
/// place. The literal-aware command framing is reused from [`dmtap_mail::net::read_imap_command`].
