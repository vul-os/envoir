<!-- GENERATED by scripts/sync-site-docs.mjs from docs/features/chat.md — do not hand-edit this file, edit the source instead. -->

# Chat

DMs and channels over the same MOTE substrate as mail — just `kind = chat` on the faster tier.
Channels are simply **groups with addresses** (see [Groups](#groups)).

![Chat — protocol badge on every conversation](assets/screens/chat-dark.png)

## Starting a conversation

Open **Chat** from the left rail. Pick an existing conversation from the list, or start a new one
from a contact's card (see [Contacts](#contacts)) or by addressing a message straight to someone
...
- **Deniable 1:1** — a pairwise X3DH/PQXDH handshake + Double Ratchet, authenticated by a
  ...
  [Privacy & threat model](#privacy#deniability-is-optional-and-11-only) for exactly what
...
[Security](#security#formal-proverif-models).
...
[Privacy & network](#settings#privacy-network)).
...
appears in the group's own audit trail. See [Groups](#groups) for creating one, adding members,
and managing roles.
...
described in [`client/README.md`](https://github.com/vul-os/envoir/blob/main/client/README.md) — see [Roadmap](#roadmap) for the
