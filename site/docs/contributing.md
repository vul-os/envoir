<!-- GENERATED by scripts/sync-site-docs.mjs from docs/contributing.md — do not hand-edit this file, edit the source instead. -->
...
- Read [architecture.md](#architecture) and [protocol.md](#protocol) for how the pieces fit
  together.
- Read [roadmap.md](#roadmap) for what's implemented vs. stubbed today, so you don't duplicate
...
See [getting-started.md](#getting-started) for the full command reference.
...
- **Grounded, not invented.** Every claim in this documentation set, and every behavior in the
  ...
  overclaiming. See [privacy.md](#privacy) and [security.md](#security) for the tone to match.
...
- **Fail closed, not open, on anything security-relevant.** DMTAP's whole downgrade-resistance
  ...
  [downgrade-tests](https://github.com/vul-os/envoir/blob/main/crates/downgrade-tests) suite, add a case there.
...
facility. See [security.md](#security#reporting-a-vulnerability) for the disclosure process and
safe-harbour terms.

## License

Everything here — protocol, node, gateway, client, libraries — ships under the **MIT license**
([`LICENSE-MIT`](https://github.com/vul-os/envoir/blob/main/LICENSE-MIT)). Apache-2.0 dual-licensing (for its explicit patent grant,
