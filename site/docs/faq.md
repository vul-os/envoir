<!-- GENERATED by scripts/sync-site-docs.mjs from docs/faq.md — do not hand-edit this file, edit the source instead. -->
...
protocol, not a production mail service. See [security.md](#security#the-audit-gate) for the
...
(see [naming.md](#naming)): the core ladder is a derived key-name, a local petname, and the
...
gateway operators. See spec §9 and [protocol.md](#protocol#anti-abuse-honestly).
...
See [features/identity.md](#identity).
...
whole way. See [features/mail.md](#mail) and
[features/transport-traceability.md](#transport-traceability).
...
the box and warms the IPs for legacy mail). See [features/self-hosting.md](#self-hosting).
...
perfectly — hardened against a *global active* one. Read [privacy.md](#privacy) for the honest,
...
as a hard gate before any production deployment. See [security.md](#security).

**What license is this under?**
Dual-licensed **MIT OR Apache-2.0** (see [`LICENSE-MIT`](https://github.com/vul-os/envoir/blob/main/LICENSE-MIT) and
[`LICENSE-APACHE`](https://github.com/vul-os/envoir/blob/main/LICENSE-APACHE) — Apache-2.0 adds an explicit patent grant). See
...
same key. See [architecture.md](#architecture) and [protocol.md](#protocol).
...
instead. See the client's own real-vs-simulated table in [`client/README.md`](https://github.com/vul-os/envoir/blob/main/client/README.md)
and [roadmap.md](#roadmap).
