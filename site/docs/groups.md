<!-- GENERATED by scripts/sync-site-docs.mjs from docs/features/groups.md — do not hand-edit this file, edit the source instead. -->
...
![Groups — an address that represents its members](assets/screens/groups-dark.png)
...
[Files & sharing](#files#removing-someones-access)) so they don't retain silent access going
forward.

## Posting to a group

Click **Post to group** from a group's header, which opens [Compose](#compose) pre-addressed to
...
- A [shared file folder](#files) **is** a group — sharing a file into a group's folder uses this
  exact membership model, roles, and re-keying.
- [Chat](#chat) channels **are** groups with a posting model of `channel`.
- Calendar meeting invitees can be drawn from a group in one step, rather than added one at a
  time — see [Calendar](#calendar#inviting-people).
