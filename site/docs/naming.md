<!-- GENERATED by scripts/sync-site-docs.mjs from docs/naming.md — do not hand-edit this file, edit the source instead. -->
...
lives in [`crates/dmtap-naming`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming) (the resolver types) and
[`crates/dmtap-core/src/keyname.rs`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-core/src/keyname.rs) (the key-name codec).
...
- Losing a name (a domain lapses, a DNS record is dropped, a chain registrar changes hands) never
  ...
  [features/identity.md](#identity#addresses-are-pointers-not-the-identity).
...
[`restype::classify`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/restype.rs).
...
landing on a different key ([`keyname::verify`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-core/src/keyname.rs)).

Resolution here is not a lookup at all — it's the forward derivation run again and compared:
[`SelfResolver::resolve`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/restype.rs) checks the checksum, re-derives
...
([`PetnameBook`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/restype.rs)). It never leaves the device and is never
a global identifier.
...
[protocol.md](#protocol#naming-key-transparency) for the full flow and
[`crates/dmtap-naming`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming)'s `dns`/`kt`/`resolver` modules for the
...
[`crates/dmtap-naming/src/namechain.rs`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/namechain.rs). Admitting a
...
1. **Optional.** `name-chain` is off by default in the resolver registry
   ([`ResolverRegistry::with_defaults`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/restype.rs) enables only
...
[`canonical::canonical_name`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/canonical.rs). The canonical form,
precisely:
...
by a UTS-39-informed **skeleton** ([`canonical::skeleton`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/canonical.rs)
...
[`ResolverRegistry`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/restype.rs); a name whose form belongs to a type
...
- A node with no domain, no registrar relationship, and no DNS resolver configured is still a
  ...
  [features/self-hosting.md](#self-hosting#running-your-own-yourdomain).
...
([`Resolver`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/resolver.rs), [`KtLog`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/kt.rs),
[`NameChainClient`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-naming/src/namechain.rs)) so the verification core is fully
unit-testable offline today, with a real network client a thin, later swap behind the identical
trait. See [security.md](#security#conformance-suite) for how naming/resolution cases are
covered in the conformance suite.

## See also

- [protocol.md](#protocol#naming-key-transparency) — naming in the context of the whole
  protocol.
- [features/identity.md](#identity) — safety numbers, the key hierarchy, and why an
  address is only ever a pointer.
- [features/self-hosting.md](#self-hosting) — the onboarding ladder from "just a key"
