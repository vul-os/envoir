<!-- GENERATED by scripts/sync-site-docs.mjs from docs/privacy.md — do not hand-edit this file, edit the source instead. -->
...
[`crates/netsim`](https://github.com/vul-os/envoir/blob/main/crates/netsim) measures exactly this: as cover-traffic rate and hop count
...
[security.md](#security#the-audit-gate).
...
- **Key-transparency freshness and mix-directory freshness are both fail-closed.** A stale or
  frozen view of either is treated as an error, not silently accepted — see [security.md](#security)
...
word list in a production client. See [features/identity.md](#identity).
...
stay on MLS and remain non-repudiable. See [features/chat.md](#chat).
...
- **Not independently audited yet.** See [security.md](#security) — an external cryptographic
  and code audit is a disclosed *gate* before any production deployment, not a checkbox already
  ticked.

## Further reading

- [security.md](#security) — six formal (ProVerif) models covering the hardest ceremonies and
  the composed group/transparency/mixing primitives, fuzzing, conformance, and the audit gate.
- [protocol.md](#protocol) — the full DMTAP protocol this page summarizes.
