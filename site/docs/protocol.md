<!-- GENERATED by scripts/sync-site-docs.mjs from docs/protocol.md — do not hand-edit this file, edit the source instead. -->
...
[`crates/conformance-runner`](https://github.com/vul-os/envoir/blob/main/crates/conformance-runner) against the spec's own conformance
catalog — see [security.md](#security#conformance-suite).
...
terminate at the same KT-verified, pinned key. See [naming.md](#naming) for the full ladder,
...
[features/identity.md](#identity) and spec §1 / §13.
...
See [features/chat.md](#chat) and [features/files.md](#files).
...
when both parties are online. See [privacy.md](#privacy) and spec §4.
...
**operator seam** (spec §12, [`crates/dmtap-seam`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-seam)) for billing and
multi-tenant management. The inviolable rule: privacy, cryptography, and recovery are never
behind that seam. See [architecture.md](#architecture#where-a-third-party-operators-tooling-sits).
...
own default. See spec §10.3 and [security.md](#security#conformance-suite).
