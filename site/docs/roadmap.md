<!-- GENERATED by scripts/sync-site-docs.mjs from docs/roadmap.md — do not hand-edit this file, edit the source instead. -->
...
- **A real libp2p mesh transport** (`crates/dmtap-p2p`) — live TCP/QUIC swarms secured by
  ...
  see [architecture.md](#architecture#the-mesh-and-mixnet).
...
- **Cryptographic primitives** (`dmtap-core`, now `kotva-core` in the sibling
  ...
  [security.md](#security#fuzzing)), the key-name checksum, delegated/attenuable capability
...
- **The web client, console, superadmin, and status apps** — fully functional UIs with real
  ...
  identicon → initials; see [features/identity.md](#identity#avatars-and-profile)) and
...
- **An installable PWA** (`client/manifest.webmanifest`, `client/sw.js`) — a real service worker
  ...
  no real push backend behind it yet. See [pwa-and-push.md](#pwa-and-push) for the full model
...
- **v1 key-transparency hardening** (federated multi-log gossip, quorum-audited bindings,
  ...
  [privacy.md](#privacy#what-this-project-does-not-claim).
- **9 of 157 conformance cases** aren't yet executed — all 9 skip with documented per-case reasons
  ...
  [security.md](#security#conformance-suite).
- **Envoir Send** — a Resend-style programmatic mail-sending API built on the delegated
  ...
  workspace. See [protocol.md](#protocol#delegated-capabilities-and-envoir-send).
...
[security.md](#security#the-audit-gate). Nothing on this page changes that.
