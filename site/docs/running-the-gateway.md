<!-- GENERATED by scripts/sync-site-docs.mjs from docs/features/running-the-gateway.md — do not hand-edit this file, edit the source instead. -->
...
[Self-hosting a node](#self-hosting) for when you do.
...
([`node/src/naming.rs`](https://github.com/vul-os/envoir/blob/main/node/src/naming.rs)) — deterministic, reversible by any gateway with
...
[Security](#security#downgrade--fail-closed-invariants).

## Billing, if you're running one for others

If you operate a gateway other people use, [transport-path provenance](#transport-traceability)
is what makes usage auditable — see
[Self-hosting a node](#self-hosting#billing-is-tied-to-the-gateway-only) for the full model
...
([`crates/dmtap-seam`](https://github.com/vul-os/envoir/blob/main/crates/dmtap-seam)) is what a hosted operator implements for metering
...
![Superadmin — fleet overview](assets/screens/superadmin-overview-dark.png)

See [`superadmin/README.md`](https://github.com/vul-os/envoir/blob/main/superadmin/README.md) for that console's own model.
