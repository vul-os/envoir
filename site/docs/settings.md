<!-- GENERATED by scripts/sync-site-docs.mjs from docs/features/settings.md — do not hand-edit this file, edit the source instead. -->
...
[Identity](#identity#devices), because it's part of the key hierarchy rather than a preference —
this page links to it where relevant.

![Settings — account, privacy, and node connection](assets/screens/settings-dark.png)

## Identity

A short summary card with a link out to the full [Identity](#identity) page — safety number,
...
safety number (see [Identity](#identity#avatars-and-profile)).
...
[Naming](#naming#name-chain-eth-sol-optional-crypto-name-chains-four-guardrails): your key
...
mechanisms described in [Protocol](#protocol#anti-abuse-honestly) replace central content
...
- **Default privacy tier** — private (mixnet, metadata-private) or fast (direct, lower latency) —
  the default [Compose & send](#compose#filling-in-the-message) starts new messages with; you
  can still override it per-message.
- **Presence & typing** — off by default because it's metadata-sensitive; see [Chat](#chat#presence-and-typing).
- **Legacy gateway** — toggle whether this client bridges to the SMTP world at all. See
  [Self-hosting a node](#self-hosting) for what running your own gateway looks like.
...
1. Run your own node (see [Getting started](#getting-started#run-the-node)) or point at one
...
3. Optionally add a **send token** — a separate, scoped, revocable capability (spec §13.5.1) that
   ...
   syncs and reads your real mailbox, but [Compose](#compose#what-the-honest-send-note-means)
...
list. See [PWA & push](#pwa-and-push) for the full model.
...
[PWA & push](#pwa-and-push#web-push-a-content-free-wake-ping-by-design) for the full
...
A working demonstration of [DMTAP-Auth](#identity#signing-in-with-your-key-dmtap-auth): sign in
...
[Identity](#identity#recovery) for what it protects and its one honest limit.
...
see [Roadmap](#roadmap) for the project-wide real-vs-simulated line.
